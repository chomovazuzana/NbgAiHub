...
**Last updated:** 2026-05-27 (overnight) — Day 1 content overhaul: rewritten public-safe to 6 ordered steps (Open terminal → Install Claude Code → GitHub account → First session → Write `CLAUDE.md` → Survival keys) sourcing the bank's WSL install doc + Microsoft official + Anthropic provider docs with every NBG-internal reference scrubbed. New side-by-side **CLAUDE.md before/after figure** in Step 5 — three distinct grey report glyphs (Without) vs three identical accent-teal glyphs (With), `loans.xlsx` + "analyse + report" pill in both. Mac terminal list reordered (cmux added first, iTerm2 moved last); Mac screenshot shortcut now `⌘C` / `control V`. Foundations CLAUDE.md bullet links to `/start-here/day-1/#d5`; intro paragraph removed. Body links now visible by default (accent + always-on underline; no more hover-to-discover). Starlight `article + article` 16px-margin gotcha hit a 3rd time on the new figure's panels — patched in-place with the same `margin: 0 !important` fix Foundations already documented on `.foundation-compare__col`. Site live at <https://chomovazuzana.github.io/NbgAiHub/>. Tests: site 310/310, pipeline 205/205. AUTO doc counts: glossary 45, tips 18, skills 6, journeys 2, news 48.
