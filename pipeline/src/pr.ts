...
/**
 * Builds the markdown body of the editorial PR. Items grouped by source
 * (feed name), sorted alphabetically by source name. Each bullet shows:
 * title, source, external_link, ai_summary.
 *
 * Pure.
 */
export function buildPrBody(items: EmittedItem[], runDateUtc: string): string {
  ...
  lines.push("");
  lines.push(`${items.length} new item${items.length === 1 ? "" : "s"} for editorial review.`);
  lines.push("");
